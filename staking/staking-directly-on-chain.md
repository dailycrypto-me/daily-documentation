...
After compilation, make sure you log into Metamask and switch the network to Tarxa Mainnet. If you aren't sure how to do that, please check out the [wallet guide on adding Daily networks into Metamask](../become-a-validator/register-node-directly-on-chain.md).&#x20;
...
* The wallet under "Account" is the wallet you want to use. Make sure you use the wallet that contains the DLY you wish to delegate.&#x20;
...
First, we enter the number of DLY to be delegated into the node. Within the "Deploy & Run Transactions" section of Remix, navigate back towards the top and you'll see a "Value" field. This field defines how many tokens will be transferred to the contract in this transaction.&#x20;
...
Now it has 2000 DLY delegation (once again, reminder there are 18 extra zeros after the number), which is exactly 1000 DLY more than before we started.&#x20;
...
In our example, since the validator only has 2000 DLY delegation, it isn't eligible to participate in consensus yet so the reward claimed was 0, which is expected.&#x20;
...
You can also remove your delegation and return the staked DLY back into your wallet by using the     `unDelegate` function. Note that, after calling the unDelegate function, it'll take 700,540 blocks (\~30 days) for the delegated tokens to unlock.&#x20;
