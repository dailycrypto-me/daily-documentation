---
description: Detailed RPC specifications for the Daily Network
---

# 🔩 Daily RPC Specs

## Daily RPC's Ethereum Compatibility

Daily is nearly 100% ETH-compatible, with a few quirks, exceptions, and Daily-specific specs.&#x20;

## Quirks of ETH-RPC on Daily

* "pending" block identifier means "latest"
* `eth_syncing` return values are to be better defined

## ETH-RPC not implemented on Daily

Daily protocol supports all Ethereum methods described on [this page](https://eth.wiki/json-rpc/API#json-rpc-methods) except:
...
## Daily-Specific RPC Methods&#x20;

### daily\_protocolVersion

Returns current daily network protocol version
...
```json
// Request
curl -X POST --data '{"jsonrpc":"2.0","method":"daily_protocolVersion","params":[],"id":1}'
...
Returns extended description of Daily node version.
...
```json
// Request
curl -X POST --data '{"jsonrpc":"2.0","method":"daily_getVersion","params":[],"id":1}'
...
```json
// Request
curl -X POST --data '{"jsonrpc":"2.0","method":"daily_getDagBlockByHash","params": ["0x73fabc5e2ff3863b8c8a313ee0200b915ef2f3606d24f3ff817a4655c04852a6", false],"id":1}'
...
```json
// Request
curl -X POST --data '{"jsonrpc":"2.0","method":"daily_getDagBlockByLevel","params":["0xEA1C6A", false],"id":1}'
...
```json
// Request
curl -X POST --data '{"jsonrpc":"2.0","method":"daily_dagBlockLevel","params":[],"id":1}'
...
```json
// Request
curl -X POST --data '{"jsonrpc":"2.0","method":"daily_dagBlockPeriod","params":[],"id":1}'
...
### daily\_getScheduleBlockByPeriod
...
```json
// Request
curl -X POST --data '{"jsonrpc":"2.0","method":"daily_getScheduleBlockByPeriod","params":["0x1D533B"],"id":1}'
...
```json
// Request
curl -X POST --data '{"jsonrpc":"2.0","method":"daily_getConfig","params":[],"id":1}'
...
```json
// Request
curl -X POST --data '{"jsonrpc":"2.0","method":"daily_getChainStats","params":[],"id":1}'
...
```json
// Request
curl -X POST --data '{"jsonrpc":"2.0","method":"daily_yield","params":["0x1D533B"],"id":1}'
...
```json
// Request
curl -X POST --data '{"jsonrpc":"2.0","method":"daily_totalSupply","params":["0x1D533B"],"id":1}'
